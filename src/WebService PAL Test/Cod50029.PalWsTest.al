codeunit 50029 "TTT-PR PalWsTest"
{
    ...
    begin
        ...
        dtStart1 := CurrentDateTime();
        for locintLoop := 1 to 100 do
            loccuMgt.Decode4Pal(loctxtInMessage, loclblMethodTxt, locdictResult);
        dtEnd1 := CurrentDateTime();
        durTime1 := dtEnd1 - dtStart1;
        //Message(loccuMgt.Dictionary2Text(locdictResult, '\'));

        dtStart2 := CurrentDateTime();
        for locintLoop := 1 to 100 do
            loccuMgt.TEMP_LoadIntoXmlBuffer();
        dtEnd2 := CurrentDateTime();
        durTime2 := dtEnd2 - dtStart2;

        Message('Dict: %1\Buff: %2', durTime1, durTime2);

        //loccuMgt.ShowXmlBuffer();

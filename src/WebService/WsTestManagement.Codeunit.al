codeunit 50027 "TTT-PR WsTestManagement"
{
    ...
    begin
        ...
        cuXml.AddNamespaceDeclaration('soap12', lblPalSoapNsUriTxt);
    ...
    begin
        ...
        if cuXml.FindChildsOfNode(loctxtXPath, parvardictValues) then
            exit(true);
        if not cuXml.FindNode(loctxtXPath) then
            exit;
        loctxtValue := cuXml.GetNodeText();
        if not parvardictValues.ContainsKey(partxtMethod) then
            parvardictValues.Add(partxtMethod, loctxtValue);
        exit(true);
    end;
    ...
    begin
        Decode4Pal(partxtXml, partxtMethod);
        loctxtXPath := StrSubstNo('soap:Envelope/soap:Body/pal:%1Response/pal:%1Result', partxtMethod);
        if cuXml.FindChildsOfNode(loctxtXPath, parvarlstValues) then
            exit(true);
        if not cuXml.FindNode(loctxtXPath) then
            exit;
        loctxtValue := cuXml.GetNodeText();
